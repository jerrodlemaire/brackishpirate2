...
export default function LocationPickerModal({
  ...
  const s = useMemo(() => StyleSheet.create({
    ...
    gpsBtn:         { backgroundColor: Colors.borderMid, borderRadius: Radius.full, paddingHorizontal: 12, paddingVertical: 5 },
    gpsTxt:         { fontSize: Typography.sm, color: Colors.textPrimary, fontWeight: '600' },
    hint:           { fontSize: Typography.xs, color: Colors.textSecondary, textAlign: 'center', paddingBottom: 6, backgroundColor: Colors.topbarBg },
    map:            { flex: 1 },
    pinEmoji:       { fontSize: 28 },
    bottom:         { backgroundColor: Colors.topbarBg, padding: Spacing.lg, gap: Spacing.md },
    locationName:   { fontSize: Typography.base, color: Colors.textPrimary, textAlign: 'center', lineHeight: 22, minHeight: 36 },
    confirmBtn:     { backgroundColor: Colors.buttonBg, borderRadius: Radius.md, paddingVertical: 14, alignItems: 'center' },
    confirmDisabled:{ opacity: 0.35 },
    confirmTxt:     { fontSize: Typography.base, fontWeight: '700', color: Colors.buttonText },
  }), [Colors])
